...
int main(int argc, char **argv) {
    ...
    float border = DEFAULT_BORDER, gain = DEFAULT_GAIN;
    ...
    while (argc > 1) {
        if (!strcmp(argv[1], "--optimize")) {
            ...
            border = atof(argv[2]);
            border = (border < 1.0) ? DEFAULT_BORDER : border;
            argv += 2; argc -= 2;

        }
        else if (argc > 2 && !strcmp(argv[1], "--gain")) {
            gain = atof(argv[2]);
            gain = (gain < 0.0) ? DEFAULT_GAIN : gain;
            argv += 2; argc -= 2;
        }
        else if (argc > 2 && !strcmp(argv[1], "--niter")) {
            niter = atoi(argv[2]);
            niter = (niter < 1) ? DEFAULT_NITER : niter;
            argv += 2; argc -= 2;
        }
...
, progname);
    ...
    logfmt("Options: border = %f, gain = %f, niter = %d\n", border, gain, niter);
    do_quantsmooth(&srcinfo, src_coef_arrays, smooth_info, border, gain, niter);
